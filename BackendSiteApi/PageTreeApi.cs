...
namespace BackendSiteApi
{
    ...
    {
        [HttpGet]
        public IReadOnlyCollection<PageTreeModel> Sites()
        {
            var sites = new SiteRepository().Find().ToList();
            return BuildPageTreeModels(sites);
        }

        private List<PageTreeModel> BuildPageTreeModels(List<Site> sites)
        {
            List<PageTreeModel> pageTreees = new List<PageTreeModel>();
            foreach (var site in sites)
            {
                var pageTree = new PageTreeModel
                {
                    SiteName = site.Name, Items = BuildPageTreeItem(site), SiteId = site.ContentId
                };
