...
namespace Modules.Cms.Features.Presentation.PageComposition.Elements
{
    public class PageDesignerHtmlFactory
    {
        ...
        private string CreateHiddenHtml(string name, object value)
            {
                ...
                return $"<input type='hidden' name='{name}' id='{name}' value='{HttpContext.Current.Server.HtmlEncode(serializedJson)}'/>";
            }
        ...
        public IEnumerable<string> CreatePageDesignerStateTags()
        {
            ...
            {
                var hiddenPassthrough = CreateHiddenHtml(clientSidePassthrough, HttpContext.Current.Request[clientSidePassthrough] ?? string.Empty);
        ...
        {
            var path = HttpContext.Current.Request.MapPath("/App_Data/PageDesignerComponents/Resources/pagedesigner-head.htm");
            return File.ReadAllText(path);
        }

        public string LoadPageDesignerStyle()
        {
            var path = HttpContext.Current.Request.MapPath("/App_Data/PageDesignerComponents/Resources/pagedesigner.css");
            return "<style>"+File.ReadAllText(path)+"</style>";
        }

        public string LoadPageDesignerScripts()
        {
            var path= HttpContext.Current.Request.MapPath("/App_Data/PageDesignerComponents/Resources/pagedesigner.js");
